...
const TVAE = () => {
  ...
  const [modelName, setModelName] = useState("tvae");
  ...
  // Handlers
  const handleFileUpload = (acceptedFiles) => setUploadedFile(acceptedFiles[0]);

  const handleExport = async (format) => {
    try {
      const res = await exportData(format, parseInt(numRows, 10));
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      setAlertType("error");
      setAlertMessage(error.response?.data?.detail || "Error during training.");
    }
  };
  ...
  const handleTrainWithCelery = async () => {
    ...
    try {
      ...
      setAlertType("error");
      setAlertMessage(error.response?.data?.detail || "Failed to submit Celery training task");
    }
  };
  ...
  const handleGenerate = async () => {
    try {
      ...
      const synthetic = await generateData(parseInt(numRows, 10));
      ...
      setAlertType("error");
      setAlertMessage(error.response?.data?.detail || "Error during data generation");
    }
  };
  ...
  const handleDownloadModel = async () => {
    ...
    try {
      ...
      setAlertType("error");
      setAlertMessage(error.response?.data?.detail || "Failed to download model");
    } finally {
  ...
  const handleLoadModel = async () => {
    ...
    try {
      ...
      setAlertType("error");
      setAlertMessage(error.response?.data?.detail || "Failed to load model");
    }
  };
  ...
  const handleSaveToDb = async () => {
    try {
      ...
      setDbRefreshKey((k) => k + 1); // uppdatera DB-vyn
    } catch (error) {
  ...
  return (
    <div>
      <IDSCard borderTop={2}>
        <h2 className="ids-heading-m">TVAE Model</h2>
        <h3 className="ids-heading-s">
          Upload your dataset to train the TVAE model, then generate synthetic data.
        </h3>
